...
export class LoremVM {
  ...
  get classicText() {
    if (this.type === "paragraphs") {
      ...
      for (let i = 0; i < this.count; i++) {
        res = res + classicText[i % classicText.length] + `\n\n`;
      }
      return res;
    } else if (this.type === "sentences") {
      const sentences = ([] as string[]).concat(...classicText.map((txt) => txt.split(".")));
      let res = [];
      for (let i = 0; i < this.count; i++) {
        res.push(sentences[i % sentences.length].trim());
      }
      return res.join(". ") + ".";
    } else if (this.type === "words") {
      const words = ([] as string[]).concat(...classicText.map((txt) => txt.split(" ")));
      let res = ``;
      for (let i = 0; i < this.count; i++) {
        res = res += " " + words[i % words.length];
      }
      return res;
    }
    return "";
  ...
  get text() {
    return this.randomize ? this.randomText : this.classicText;
  }

  serialize() {
    return { type: this.type, count: this.count, randomize: this.randomize };
  }
  ...
  deserialize(data: any) {
    ...
    this.setRandomize(!!data.randomize);
  }
}
